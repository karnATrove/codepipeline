...
use WarehouseBundle\Workflow\IncomingWorkflow;
...
class ScanController extends Controller
{
	...
	public function indexAction(Request $request)
	{
		...
		return $this->render('WarehouseBundle::Scan/index.html.twig', [
	...
	public function scanModeAuto($search_string)
	{
		...
		$response['ajaxCommand'][] = [
			...
			'op' => 'html',
			'value' => $this->renderView('WarehouseBundle::Scan/scan_result.html.twig', [
				'search' => $search_string,
				'product' => ['count' => count($products), 'results' => $products],
				'booking' => ['count' => count($bookings), 'results' => $bookings],
				'incoming' => ['count' => count($incoming), 'results' => $incoming],
				'return' => ['count' => count($returns), 'results' => $returns],
			]),
		];
	...
	public function scanModeStock($incomingId)
	{
		...
		if (!$incoming || !IncomingManager::isActive($incoming)) {
			$response['ajaxCommand'][] = [
				'selector' => '#scan-result',
				'op' => 'html',
				'value' => $this->renderView('WarehouseBundle::Scan/stock/error.html.twig', [
					'search' => $incomingId,
		...
		$html = $this->renderView('WarehouseBundle::Scan/stock/incoming.html.twig', [
			'form' => $form->createView(),
	...
	public function scanModePick($search_string)
	{
		...
		try {
			...
			$html = $this->renderView('WarehouseBundle::Scan/booking/pick.html.twig', [
				'form' => $form->createView(),
				'search' => $search_string,
				'booking' => $booking,
				'notice' => $notice,
			...
			$html = $this->renderView('WarehouseBundle::Scan/booking/error.html.twig', ['search' => $search_string, 'error' => $e->getMessage()]);
		}
		$response = [];
		$response['ajaxCommand'][] = [
			'selector' => '#scan-result',
	...
	public function scanModeInventory($search_string)
	{
		...
		$html = $this->renderView('WarehouseBundle::Scan/inventory/location.html.twig', [
			'form' => $form->createView(),
			'search' => $search_string,
			'notice' => $notice,
		]);
	...
	public function scanModeOrder($search_string)
	{
		...
		# Response back with matched results
		$response = [];
		$html = $this->renderView('WarehouseBundle::Scan/booking/order.html.twig', [
			//'form' => $form->createView(),
			'search' => $search_string,
			'bookings' => $bookings,
		]);
	...
	public function formAjaxBookingProductPickForm(Request $request, Booking $booking)
	{
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			$response['ajaxCommand'][] = [
				'selector' => '#scan-result',
				'op' => 'html',
				'value' => $this->renderView('WarehouseBundle::Scan/booking/complete.html.twig', ['booking' => $booking]),
			];
		} else {
			$response['ajaxCommand'][] = [
				'selector' => '#scan-result',
				'op' => 'prepend',
				'value' => $this->renderView('WarehouseBundle::Scan/booking/error.html.twig', ['search' => $booking->getOrderNumber(), 'error' => 'There was an issue trying to submit the form.']),
			];
		}
	...
	public function formAjaxStockIncomingForm(Request $request, Incoming $incoming)
	{
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			$response['ajaxCommand'][] = [
				...
				'op' => 'html',
				'value' => $this->renderView('WarehouseBundle::Scan/stock/product_scans.html.twig', [
	...
	public function formAjaxStockIncomingDeleteScan(Request $request, IncomingProductScan $incomingProductScan)
	{
		...
		if ($incoming->getStatus(1, 2)) { # Inbound or Arrived
			...
			$form_scan = $this->createModifyForm($incoming);
			$html = $this->renderView('WarehouseBundle::Scan/stock/product_scans.html.twig', [
	...
	public function formAjaxStockIncomingProductForm(Request $request, Incoming $incoming)
	{
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			if ($form->get('complete')->isClicked()) {
				try {
					$this->get(IncomingWorkflow::class)->setIncomingComplete($incoming);
	...
	function createModifyForm(Incoming $incoming)
	{
		$form = $this->createFormBuilder($incoming)
			->setAction($this->generateUrl('scan_stock_product_ajax', ['id' => $incoming->getId()]))
			->setMethod('POST')
	...
	public function createBookingProductPickForm(Request $request, Booking $booking)
	{
		return $this->createFormBuilder($booking)
			->setAction($this->generateUrl('scan_booking_ajax', ['id' => $booking->getId()]))
			->setMethod('POST')
	...
	public function createIncomingItemStockForm(Request $request, Incoming $incoming)
	{
		return $this->createFormBuilder()
			...
			->add('scanIncoming', TextType::class, [
				'attr' => [
	...
	public function createInventoryStockForm(Request $request)
	{
		return $this->createFormBuilder()
			...
			->add('scanInventory', TextType::class, [
				'attr' => [
			...
			->add('scanLocation', TextType::class, [
				'attr' => [
